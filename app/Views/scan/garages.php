...
<div id="locationPromptModal" class="settings-modal-overlay">
    <div class="settings-modal-card" style="text-align: center;">
        ...
        <p style="font-size: 0.88rem; color: #64748b; margin-bottom: 1.35rem; line-height: 1.5;">
            Vehicle Sampark requires your location to send to backend API and search nearest garages.
        </p>
    ...
    <div class="content-card">
        ...
        <div style="text-align: center; padding-bottom: 1.25rem; border-bottom: 1px solid var(--border-color); margin-bottom: 1.25rem;">
            ...
            <p style="color: var(--text-muted); font-size: 0.88rem; margin-bottom: 0; max-width: 600px; margin: 0 auto; line-height: 1.45;">
                Backend API processes your location coordinates & searches nearest multi-brand garages with direct Google Maps turn-by-turn navigation.
            </p>
        ...
        <div id="locationStatusBox" style="background: #f0f9ff; border: 1.5px solid #bae6fd; padding: 1rem 0.85rem; border-radius: 16px; margin-bottom: 1.25rem; text-align: center;">
            <div id="locationStatusText" style="font-weight: 800; color: #0369a1; font-size: 0.95rem; display: flex; align-items: center; justify-content: center; gap: 0.4rem; flex-wrap: wrap;">
                <i class="fa-solid fa-compass fa-spin" style="font-size: 1.2rem; color: #0284c7;"></i> Requesting Location...
            </div>
            <div id="locationSubText" style="font-size: 0.82rem; color: #0284c7; margin-top: 0.3rem; line-height: 1.4;">
                Please tap <strong>ALLOW LOCATION ACCESS</strong> to send location to backend API and search nearest garages.
            </div>
            ...
            <div id="gpsActionArea" style="margin-top: 0.75rem; display: flex; flex-direction: column; gap: 0.5rem; justify-content: center;">
                <button id="enableGpsBtn" class="btn btn-primary" style="width: 100%; padding: 0.75rem 1rem; font-size: 0.9rem; font-weight: 800; background: linear-gradient(135deg, #0284c7, #0369a1); border: none; border-radius: 12px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 0.4rem; box-shadow: 0 6px 18px rgba(2, 132, 199, 0.3);">
                    <i class="fa-solid fa-location-dot" style="font-size: 1rem;"></i> 📍 Send Location to Backend & Search Garages
                </button>
        ...
        <div id="garagesList" class="garage-grid">
            <!-- Rendered dynamically via JavaScript from Backend API response -->
        </div>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Fetch nearest garages dataset directly from Backend PHP API (api/nearest_garages.php)
    function fetchGaragesFromBackend(userLat, userLng) {
        let apiUrl = 'api/nearest_garages.php';
        if (userLat && userLng) {
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                if (data.success && data.garages) {
                    renderGaragesFromBackend(data.garages);
                }
            })
            .catch(err => {
                console.error("Backend Garages API Error:", err);
            });
    }

    function renderGaragesFromBackend(garages) {
        ...
        garages.forEach(garage => {
            ...
            card.innerHTML = `
                <div>
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.4rem; gap: 0.4rem;">
                        ...
                        <span style="background: #e0f2fe; color: #0369a1; font-size: 0.75rem; font-weight: 800; padding: 0.2rem 0.55rem; border-radius: 20px; white-space: nowrap; flex-shrink: 0;">
                            <i class="fa-solid fa-location-arrow"></i> ${garage.distance_text}
                        </span>
                    </div>
                    ...
                    <div style="font-size: 0.78rem; color: #64748b; margin-bottom: 0.65rem; display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap;">
                        ...
                        <span style="color: #16a34a; font-weight: 700;">• ${garage.open_status}</span>
                    </div>
                ...
                <div class="garage-action-btn-grid" style="margin-top: auto;">
                    <a href="${garage.directions_url}" target="_blank" class="btn" style="padding: 0.75rem 0.4rem; font-size: 0.82rem; background: #0284c7; color: #ffffff; font-weight: 700; border-radius: 10px; text-decoration: none; display: flex; align-items: center; justify-content: center; gap: 0.35rem; box-shadow: 0 4px 10px rgba(2, 132, 199, 0.25);">
        ...
        fetch("https://ipapi.co/json/")
            ...
            .then(data => {
                if (data.latitude && data.longitude) {
                    ...
                    locationStatusText.innerHTML = `<i class="fa-solid fa-circle-check" style="color: #10b981; font-size: 1.2rem;"></i> Location Received (${city}): ${lat}° N, ${lng}° E`;
                    locationSubText.style.color = "#047857";
                    locationSubText.innerHTML = "Backend API calculated nearest garages. Tap <strong>Directions</strong> for Google Maps turn-by-turn navigation.";
                    ...
                    fetchGaragesFromBackend(lat, lng);
                } else {
                    fetchGaragesFromBackend(null, null);
                }
            })
            .catch(() => {
                fetchGaragesFromBackend(null, null);
    ...
    function triggerHardwareGps() {
        ...
        locationStatusText.innerHTML = `<i class="fa-solid fa-satellite-dish fa-spin" style="font-size: 1.2rem; color: #0284c7;"></i> Sending Location Input to Backend API...`;
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    locationStatusBox.style.borderColor = "#a7f3d0";
                    locationStatusText.style.color = "#065f46";
                    locationStatusText.innerHTML = `<i class="fa-solid fa-circle-check" style="color: #10b981; font-size: 1.2rem;"></i> Backend Input GPS: ${lat}° N, ${lng}° E`;
                    locationSubText.style.color = "#047857";
                    locationSubText.innerHTML = "Backend API processed coordinates & sorted nearest garages. Tap <strong>Directions</strong> for Google Maps navigation.";
                    gpsActionArea.style.display = "none";
